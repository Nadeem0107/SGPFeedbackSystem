...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(CFSView.WELCOME_CTL, request, response);
			return;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				UserBean user = model.authenticate(bean.getUserName(), bean.getPassword());
				String requestedRoleName = bean.getRoleName();

				if (user != null) {
					// Strict Role Name Validation
					String actualRoleName = user.getRoleName();

					if (requestedRoleName != null && actualRoleName != null) {
						boolean authorized = false;

						if ("Student".equalsIgnoreCase(requestedRoleName)) {
							authorized = "Student".equalsIgnoreCase(actualRoleName);
						} else if ("HOD".equalsIgnoreCase(requestedRoleName)) {
							authorized = "HOD".equalsIgnoreCase(actualRoleName);
						} else if ("Admin".equalsIgnoreCase(requestedRoleName)) {
							// Admin tab is ONLY for Principal
							authorized = "Principal".equalsIgnoreCase(actualRoleName);
						} else if ("Principal".equalsIgnoreCase(requestedRoleName)) {
							authorized = "Principal".equalsIgnoreCase(actualRoleName);
						}

						if (!authorized) {
							// Role Mismatch or Unauthorized
							bean = (UserBean) populateBean(request);
							ServletUtility.setBean(bean, request);
							String errorMsg = "Unauthorized login. ";
							if ("Admin".equalsIgnoreCase(requestedRoleName)) {
								errorMsg += "Admin login is only for Principal.";
							} else {
								errorMsg += "You selected " + requestedRoleName + " tab but your account is "
										+ actualRoleName + ".";
							}
							ServletUtility.setErrorMessage(errorMsg, request);
							ServletUtility.forward(getView(), request, response);
							return;
						}
					}
			...
			ServletUtility.redirect(CFSView.USER_REGISTRATION_CTL, request, response);
			return;
		}
