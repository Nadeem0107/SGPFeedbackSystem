...
import in.co.customer.feedback.sys.ctl.BaseCtl;
...
public class ServletUtility {
	...
	public static void forward(String page, HttpServletRequest request, HttpServletResponse response)
		...
		String path = page;
		if (path.startsWith(CFSView.APP_CONTEXT)) {
			path = path.substring(CFSView.APP_CONTEXT.length());
		}
	...
	public static void handleException(Exception e, HttpServletRequest request, HttpServletResponse response)
		...
		ServletUtility.forward(CFSView.ERROR_CTL, request, response);
		e.printStackTrace();
	}
	...
	public static String subImage(HttpServletRequest request, HttpServletResponse response, String name)
		...
		System.out.println("Uploading image to path: " + savePath);
		File fileSaveDir = new File(savePath);
		if (!fileSaveDir.exists()) {
			boolean created = fileSaveDir.mkdirs();
			System.out.println("Created directory " + savePath + " : " + created);
		}

		Part part = request.getPart("image");
		if (part == null || part.getSize() == 0) {
			System.out.println("No image part found or it is empty.");
			return "";
		}

		String fileName = extractFileName(part);
		if (fileName == null || fileName.isEmpty()) {
			System.out.println("Could not extract filename from part.");
			return "";
		}

		// Ensure name is clean
		String cleanName = DataUtility.removeSpace(name);
		String saveFileName = cleanName + "_" + System.currentTimeMillis() + "_" + fileName;

		System.out.println("Saving file as: " + saveFileName);
	...
	public static String subFile(HttpServletRequest request, HttpServletResponse response, String name)
		...
		String fileName = extractFileName(part);
		String saveName = DataUtility.removeSpace(name) + "_" + System.currentTimeMillis() + "_"
				+ DataUtility.removeSpace(fileName);
