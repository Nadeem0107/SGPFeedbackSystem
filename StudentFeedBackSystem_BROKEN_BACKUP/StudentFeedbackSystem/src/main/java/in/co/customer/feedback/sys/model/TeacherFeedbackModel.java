...
public class TeacherFeedbackModel {
    ...
    public long add(TeacherFeedbackBean bean) throws ApplicationException {
        ...
        try {
            ...
            pstmt.setTimestamp(11, bean.getCreatedDatetime()); // created_at equivalent often, but purely matching
                                                               // schema placeholders
    ...
    public List<TeacherFeedbackBean> search(TeacherFeedbackBean bean) throws ApplicationException {
        return search(bean, 0, 0, 0);
    }

    public List<TeacherFeedbackBean> search(TeacherFeedbackBean bean, int pageNo, int pageSize)
            throws ApplicationException {
        return search(bean, pageNo, pageSize, 0);
    }

    public List<TeacherFeedbackBean> search(TeacherFeedbackBean bean, int pageNo, int pageSize, long deptId)
            throws ApplicationException {
        log.info("Model search Started");
        StringBuffer sql = new StringBuffer(
                "SELECT f.* FROM st_feedback f JOIN teachers t ON f.TEACHER_ID = t.ID WHERE 1=1");

        if (bean != null) {
            if (bean.getTeacherId() > 0) {
                sql.append(" AND f.TEACHER_ID = " + bean.getTeacherId());
            }
            if (bean.getStudentName() != null && bean.getStudentName().length() > 0) {
                sql.append(" AND f.STUDENT_NAME like '" + bean.getStudentName() + "%'");
            }
        }
