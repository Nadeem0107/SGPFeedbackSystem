			...
			try {
				...
				if (user != null) {
					...
					if (requestedRoleName != null && actualRoleName != null
							&& !requestedRoleName.equalsIgnoreCase(actualRoleName)) {
						// Role Mismatch
						bean = (UserBean) populateBean(request);
						ServletUtility.setBean(bean, request);
						ServletUtility.setErrorMessage("Unauthorized login. You selected " + requestedRoleName
								+ " tab but your account is registered as " + actualRoleName
								+ ". Please use the correct tab.",
								request);
						ServletUtility.forward(getView(), request, response);
						return;
					}
