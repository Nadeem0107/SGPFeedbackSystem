...
public class MyProfileCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0 || op != null) {
			...
			try {
				...
				if (bean.getDeptId() > 0) {
					in.co.customer.feedback.sys.model.DepartmentModel deptModel = new in.co.customer.feedback.sys.model.DepartmentModel();
					in.co.customer.feedback.sys.bean.DepartmentBean deptBean = deptModel.findByPK(bean.getDeptId());
					if (deptBean != null) {
						request.setAttribute("deptName", deptBean.getName());
					}
				}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SAVE.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(CFSView.CHANGE_PASSWORD_CTL, request, response);
			return;
		}
		ServletUtility.forward(getView(), request, response);
	...
	protected String getView() {
		...
		return CFSView.MY_PROFILE_VIEW;
	}
